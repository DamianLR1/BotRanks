...
async function backfillStorePackages(channelId, guildId) {
    try {
        ...
        let totalCount = 0; let lastId; let messagesFetched = 0; const batchSize = 100;
        const maxPackagesToFind = 115; const maxStaleMessages = 500; let messagesSinceLastFind = 0;
        ...
        while (true) {
            ...
            messages.every(message => {
                ...
                } if (totalCount >= maxPackagesToFind) return false; return true;
            });
...
async function processWebhookMessage(message) {
    ...
    if (matchTotal) {
        ...
        try { await pool.query(`INSERT INTO clan_stats (guild, total_puntos) VALUES ($1, $2) ON CONFLICT (guild) DO UPDATE SET total_puntos = $2`, [guildId, totalPuntos]); console.log(`[PROCESS] 🔵 Total actualizado: ${totalPuntos} (ID: ${message.id})`); }
        catch (err) { console.error('[PROCESS] ❌ Error al guardar puntos totales:', err); }
    }
    const matchTienda = description.match(/Tienda de Almas/i);
    if (matchTienda) {
         try { await pool.query(`INSERT INTO clan_stats (guild, paquetes_tienda) VALUES ($1, 1) ON CONFLICT (guild) DO UPDATE SET paquetes_tienda = clan_stats.paquetes_tienda + 1`, [guildId]); console.log(`[PROCESS] 📦 Paquete de tienda detectado! (ID: ${message.id})`); }
         catch (err) { console.error('[PROCESS] ❌ Error al guardar paquete de tienda:', err); }
    }
}
...
async function syncRecentPoints(channelId, guildId) {
    ...
    try {
        ...
        if (result.rows.length > 0 && result.rows[0].last_processed_message_id) { lastProcessedId = result.rows[0].last_processed_message_id; console.log(`[SYNC] Último ID en DB: ${lastProcessedId}`); }
        else { console.log(`[SYNC] Usando RESET_MESSAGE_ID: ${lastProcessedId}`); }
        if (!lastProcessedId) { console.warn('[SYNC] ⚠️ No hay ID para sincronizar.'); return; }
        const channel = await client.channels.fetch(channelId); if (!channel || !channel.messages) { console.error(`[SYNC] ❌ Canal ${channelId} no encontrado.`); return; }
        let newMessages = []; let currentLastId = lastProcessedId; let newestMessageIdInSync = lastProcessedId;
        console.log(`[SYNC] Buscando mensajes NUEVOS después de ${currentLastId}...`);
        while (true) {
            const messages = await channel.messages.fetch({ limit: 100, after: currentLastId }); if (messages.size === 0) { console.log(`[SYNC] No hay mensajes más nuevos.`); break; }
            messages.forEach(msg => { newMessages.push(msg); if (BigInt(msg.id) > BigInt(newestMessageIdInSync)) newestMessageIdInSync = msg.id; });
            currentLastId = messages.first().id; console.log(`[SYNC] ... ${newMessages.length} nuevos encontrados. Último ID en batch: ${currentLastId}`);
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  if (interaction.isChatInputCommand()) {
    ...
    if (interaction.commandName === 'rankclan') {
        ...
        const fetchAndDisplay = async (page) => { try { const offset = page * pageSize; const result = await pool.query('SELECT usuario, puntos FROM puntos WHERE guild = $1 ORDER BY puntos DESC LIMIT $2 OFFSET $3', [interaction.guild.id, pageSize, offset]); if (!result.rows.length) return interaction.editReply({ content: '⚠️ No hay puntos.' }); const lines = result.rows.map((row, i) => `${offset + i + 1}. **${row.usuario}** — ${row.puntos} pts`); const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('prev_page_cmd').setLabel('⬅️').setStyle(ButtonStyle.Primary).setDisabled(page === 0), new ButtonBuilder().setCustomId('next_page_cmd').setLabel('➡️').setStyle(ButtonStyle.Primary).setDisabled(page >= totalPages - 1)); await interaction.editReply({ content: `🏆 **Ranking (Pág ${page + 1}/${totalPages}):**\n\n${lines.join('\n')}`, components: [row] }); } catch (err) { console.error(err); if (interaction.replied || interaction.deferred) await interaction.editReply({ content: '❌ Error.', components: [] });}};
